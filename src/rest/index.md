...
A **resource** is anything the system can manage: files on disk, rows/documents in a database, the shared data of a multi-player game, or even controllers for physical systems. A web server may be able to manage many different kinds of resources, and our API needs to allow programmers to refer to specific resources in a coherent manner.

Each of these resources has a **state** of some sort, which is the current value of the resource. The state for a file on disk is the file's contents. The state for a row in a database are the values of the various columns in that row. The state for shared data in a multi-player game would be the positions, status, and actions of all the various players and bots. The state for a controller of a physical system would be the various settings and sensor readings captured by the controller. The state of a resource can be structured and complex, but ultimately it's a series of bytes that capture the current value of the entire resource.
...
When we apply this REST pattern to the design of web-based APIs that manage data in persistent DBMSs, the result is known as a **RESTful Web API**.

### Resources

The resources managed by such an API are primarily rows or documents in a persistent DBMS, not files on disk, so we need to decide how our resource paths map to tables/collections and rows/documents. The convention used by many APIs follows this pattern:
...
The `/collection` segment names a collection or table within your DBMS. For example `/v1/users` would refer to all user profiles managed by your system.

The `/unique-identifier` segment names a specific document or row within the specified collection or table. This identifier is typically the primary key value, but it could be some other value that is unique within the collection, or a special moniker that can be resolved from context. For example `/v1/users/1234` would refer to the user profile with the ID `1234`, while `/v1/users/me` would refer to the currently authenticated user.

The `/related-collection` segment can be used to identify a subset of data related to a specific row or document. For example, `/v1/photos` would refer to all photos managed by the system, but `/v1/users/me/photos` would refer to all photos uploaded by me. And `/users/` would refer to all users profiles, but `/v1/users/me/friends` would refer to only those users in my friends list.
